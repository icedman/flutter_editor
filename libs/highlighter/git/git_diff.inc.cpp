    ...
    diff_opts *opts)
{
    ...
    if (l->origin == GIT_DIFF_LINE_CONTEXT || l->origin == GIT_DIFF_LINE_ADDITION || l->origin == GIT_DIFF_LINE_DELETION) {
        if (fp) fputc(l->origin, fp);
        ss << l->origin;
    }

    // git diff path_spec:git_diff.inc.cpp
    if (fp) fwrite(l->content, 1, l->content_len, fp);
...
static void diff_print_stats(git_diff* diff, struct diff_opts* o, request_t *req)
{
...
void treeish_to_tree(
    git_tree** out, git_repository* repo, const char* treeish, request_t *req)
{
    git_object* obj = NULL;
    int error = 0;
...
int _git_diff(Json::Value json, request_t *req)
{
    ...
    struct diff_opts o = {
        ...
        -1, 0, 0, GIT_DIFF_FORMAT_PATCH, NULL, NULL, pathSpec.c_str(), pathSpec.length() == 1
    };
